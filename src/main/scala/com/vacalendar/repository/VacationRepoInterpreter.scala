...
object VacationSQL {
  ...
  def selectEmplVacsCurrYear(emplId: Long): Query0[Vacation] = {
    val currY = Year.now()
  ...
  def selectOverlappedPosIdVacsCurrYear(posId: Long,
                                       vacStart: LocalDate,
                                       vacEnd: LocalDate): Query0[Vacation] = {

      val currYear = Year.now()
...
class VacationRepoInterpreter[F[_]](val xa: Transactor[F])
  ...
  def createVac(emplId: Long, vacIn: VacationIn): EitherT[F, AppError, Vacation] =
    VacationSQL.insertVac(emplId, vacIn).unique
      .transact(xa)
      .attemptT
      .leftMap[AppError](AppError.DbErrWrapper)

  def updateVac(emplId: Long, vacId: Long, vacIn: VacationIn): EitherT[F, AppError, Option[Vacation]] =
    VacationSQL.updateVac(emplId, vacId, vacIn).option
      .transact(xa)
      .attemptT
      .leftMap[AppError](AppError.DbErrWrapper)
}